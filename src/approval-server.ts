...
export async function startApprovalServer(plan: TestPlan, context: { applicationName?: string; target: string; environment: string; scope: string[] }): Promise<ApprovalServer> {
  const reviewToken = randomBytes(24).toString("base64url"), csrf = randomBytes(24).toString("base64url"), scriptNonce = randomBytes(18).toString("base64url"), planHash = stableHash(plan);
  ...
  const server: Server = createServer(async (request, response) => {
    ...
    const send = (code: number, body: string, type = "text/html") => response.writeHead(code, { "content-type": `${type}; charset=utf-8`, "content-security-policy": `default-src 'self'; script-src 'nonce-${scriptNonce}'; style-src 'unsafe-inline'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'`, "cache-control": "no-store" }).end(body);
    if (!equal(url.searchParams.get("token") ?? "", reviewToken)) return send(403, "Review token is invalid or expired.", "text/plain");
    if (request.method === "GET" && url.pathname === "/") return send(200, page(plan, planHash, csrf, scriptNonce, context, reviewToken));
...
export function page(plan: TestPlan, planHash: string, csrf: string, scriptNonce: string, context: { applicationName?: string; target: string; environment: string; scope: string[] }, token: string): string {
